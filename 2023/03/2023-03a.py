...
def main(input: str) -> int | str | None:
    ...
    numbers = {}
    numbers_coords = {}
    parts_coords: set[tuple[int, int]] = set()
    ...
    for y, line in enumerate(lines):
        ...
        for x, c in enumerate(line):
            if c == ".":
                continue
            parts_coords |= set(itertools.product(range(x - 1, x + 2), range(y - 1, y + 2)))

    adj = set()
    for coord in numbers_coords.keys() & parts_coords:
        adj.add(numbers_coords[coord])

    return sum(numbers[adj_] for adj_ in adj)
