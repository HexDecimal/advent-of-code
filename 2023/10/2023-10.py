...
def main(input: str) -> int | str | None:
    ...
    pipe_shape = np.repeat(np.repeat(dist < 0, repeats=2, axis=0), repeats=2, axis=1)
    pipe_shape[1::2, 1::2] = 1
    for y, x in itertools.product(range(pipe_map.shape[0]), range(pipe_map.shape[1])):
        if (1, 0) not in PIPES[pipe_map[y, x]]:
            pipe_shape[y * 2 + 1, x * 2] = 1
        if (0, 1) not in PIPES[pipe_map[y, x]]:
            pipe_shape[y * 2, x * 2 + 1] = 1

    labels, count = scipy.ndimage.label(pipe_shape)
    total = 0
    for i in range(1, count + 1):
        label = labels == i
        if label[-1, :].any() or label[0, :].any() or label[:, -1].any() or label[:, 0].any():
            continue
        label = label[0::2, 0::2]
        print(f"{i=}, {label.sum()=}")
        total += label.sum()
