...
def main(input: str) -> (int | str | None):
    ...
    target_yx_ = walls.shape[0] - 1, walls.shape[1] - 2
    assert not walls[target_yx_]

    phase_distance = target_yx_[0] + target_yx_[1] - 1
    target_yx = [target_yx_, (0, 1), target_yx_]

    heap = [(0, State(turn=0, y=0, x=1, phase=0))]
    touched = set()
    ...
    while heap:
        ...
        for dy, dx in ((-1, 0), (1, 0), (0, -1), (0, 1), (0, 0)):
            ...
            if (next_y, next_x) == target_yx[state.phase]:
                if state.phase == 2:
            ...
            distance = (
                abs(target_yx[state.phase][0] - next_y)
                + abs(target_yx[state.phase][1] - next_x)
                + (2 - next_phase) * phase_distance
            )
            if distance < best_distance:
                best_distance = distance
                print(distance, next_turn, (next_x, next_y, next_phase))
            next_state = State(next_turn, next_y, next_x, next_phase)
